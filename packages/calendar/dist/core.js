...
import { useContext, useState, useEffect, useMemo, useRef as useRef$1 } from 'preact/hooks';
...
function AppWrapper({ $app }) {
    ...
    return (jsx(Fragment, { children: jsx("div", { ref: elementRef, className: classList.join(' '), children: jsxs(AppContext$1.Provider, { value: $app, children: [jsx(AppInput, {}), $app.datePickerState.isOpen.value && appPopupJSX] }) }) }));
}
...
function TodayButton() {
    ...
    return (jsx("button", { type: "button", className: 'sx__button sx__today-button sx__ripple', onClick: setToday, children: $app.translate('Today') }));
}
...
function TimeAxis() {
    ...
    return (jsx(Fragment, { children: jsx("div", { className: "sx__week-grid__time-axis", children: gridSteps.map((gridStep, index) => (jsxs("div", { className: "sx__week-grid__hour", children: [hourCustomComponentFn && hourCCIDs.length && (jsx("div", { "data-ccid": hourCCIDs[index] })), !hourCustomComponentFn && (jsx("span", { className: "sx__week-grid__hour-text", children: formatter.format(new Date(0, 0, 0, gridStep.hour, gridStep.minute)) }))] }))) }) }));
}
...
function DateGridEvent({ calendarEvent, gridRow, isCopy, }) {
    ...
    const customComponentId = useRef$1(customComponent
...
function useFetchEvents($app) {
    ...
    const initialRangeRef = useRef$1(rangeToString($app.calendarState.range.value));
    const fetchAndSetEvents = async () => {
        var _a;
        if (!((_a = $app.config.callbacks) === null || _a === void 0 ? void 0 : _a.fetchEvents) ||
            !$app.calendarState.range.value) {
            return;
        }
...
function MonthGridDay({ day, isFirstWeek, isLastWeek }) {
    ...
    return (jsxs("div", { className: wrapperClasses.join(' '), "data-date": toDateString$1(day.date), onClick: (e) => $app.config.callbacks.onClickDate &&
            $app.config.callbacks.onClickDate(day.date, e), "aria-label": getLocalizedDate$1(day.date, $app.config.locale.value), onDblClick: (e) => { var _a, _b; return (_b = (_a = $app.config.callbacks).onDoubleClickDate) === null || _b === void 0 ? void 0 : _b.call(_a, day.date, e); }, onMouseDown: handleMouseDown, children: [fullDayBackgroundEvent && (jsx(Fragment, { children: jsx("div", { className: "sx__month-grid-background-event", title: fullDayBackgroundEvent.title, style: {
                ...
                }) }), numberOfNonDisplayedEvents > 0 ? (jsx("button", { type: "button", className: "sx__button sx__month-grid-day__events-more sx__ripple--wide", "aria-label": getAriaLabelSingularOrPlural(numberOfNonDisplayedEvents), onClick: handleClickAdditionalEvents, children: getEventTranslationSingularOrPlural(numberOfNonDisplayedEvents) })) : null] }));
}
...
const performRangeExpansion = ({ $app, wrapperRef, lastDateInList, lastRangeExpansionRef, isExpandingRangeRef, scrollPositionBeforeExpansionRef, }) => {
    ...
    if (oneYearFromLast.epochNanoseconds > currentRange.end.epochNanoseconds &&
...
const expandInfiniteRecurringEventsIfNeeded = ({ $app, wrapperRef, filteredEvents, lastRangeExpansionRef, isExpandingRangeRef, scrollPositionBeforeExpansionRef, }) => {
    if (!hasInfiniteRecurringEvents(filteredEvents) ||
...
const EventTimes = ({ isFirstDay, isLastDay, isMultiDay, endLocaleString, startLocaleString, }) => {
    if (!isMultiDay) {
        return (jsxs(Fragment, { children: [jsx("div", { className: "sx__list-event-start-time", children: startLocaleString }), endLocaleString && (jsx("div", { className: "sx__list-event-end-time", children: endLocaleString }))] }));
    }
    if (isFirstDay) {
        return (jsxs(Fragment, { children: [jsx("div", { className: "sx__list-event-start-time", children: startLocaleString }), jsx("div", { className: "sx__list-event-arrow", children: "\u2192" })] }));
    }
    if (isLastDay) {
        return (jsxs(Fragment, { children: [jsx("div", { className: "sx__list-event-arrow", children: "\u2190" }), jsx("div", { className: "sx__list-event-end-time", children: endLocaleString })] }));
    }
    return jsx("div", { className: "sx__list-event-arrow", children: "\u2194" });
};

function ListEvent({ calendarEvent, eventInstanceInfo, onEventClick, onEventDoubleClick, onEventKeyDown, }) {
    var _a, _b, _c;
    const $app = useContext(AppContext);
    const customListEventComponent = $app.config._customComponentFns.listEvent;
    const customComponentId = useRef$1(customListEventComponent
        ? 'custom-list-event-' + randomStringId()
        : undefined);
    useEffect(() => {
        if (!customListEventComponent)
            return;
    ...
    const hasCustomContent = (_a = calendarEvent._customContent) === null || _a === void 0 ? void 0 : _a.listEvent;
    ...
    return (jsxs("div", { className: classNames.join(' '), "data-ccid": customComponentId.current, "data-event-id": calendarEvent.id, style: {
        ...
        }, onClick: (e) => onEventClick(e, calendarEvent), onDblClick: (e) => onEventDoubleClick(e, calendarEvent), onKeyDown: (e) => onEventKeyDown(e, calendarEvent), tabIndex: 0, role: "button", children: [!customListEventComponent && !hasCustomContent && (jsxs(Fragment$1, { children: [jsx("div", { className: `sx__list-event-color-line`, style: {
                            backgroundColor: `var(--sx-color-${calendarEvent._color})`,
                        } }), jsxs("div", { className: "sx__list-event-content", children: [jsx("div", { className: "sx__list-event-title", children: calendarEvent.title }), jsx("div", { className: "sx__list-event-times", children: jsx(EventTimes, { ...eventInstanceInfo }) })] })] })), hasCustomContent && (jsx("div", { dangerouslySetInnerHTML: {
                    __html: ((_c = calendarEvent._customContent) === null || _c === void 0 ? void 0 : _c.listEvent) || '',
                } }))] }));
}
...
const ListWrapper = ({ $app, id, }) => {
    ...
    const { setClickedEvent } = useEventInteractions($app);
    const listDayHeaderCustomComponentFn = $app.config._customComponentFns.listDayHeader;
    const listNoEventsCustomComponentFn = $app.config._customComponentFns.listNoEvents;
    const listNoEventsCCID = useRef$1(listNoEventsCustomComponentFn
    ...
    useEffect(() => {
        ...
        if (isExpandingRangeRef.current &&
    ...
    useEffect(() => {
        if (!listNoEventsCustomComponentFn || daysWithEvents.length > 0)
            return;
        ...
        return () => {
            var _a, _b;
            (_b = (_a = $app.config)._destroyCustomComponentInstance) === null || _b === void 0 ? void 0 : _b.call(_a, listNoEventsCCID.current);
        };
    }, [daysWithEvents.length, listNoEventsCustomComponentFn]);
    useEffect(() => {
        if (!listDayHeaderCustomComponentFn)
            return;
        ...
        return () => {
            Object.values(dayHeaderCCIDsRef.current).forEach((ccid) => {
                var _a, _b;
                (_b = (_a = $app.config)._destroyCustomComponentInstance) === null || _b === void 0 ? void 0 : _b.call(_a, ccid);
            });
        };
    ...
    return (jsx(AppContext.Provider, { value: $app, children: jsx("div", { id: id, className: "sx__list-wrapper", ref: wrapperRef, children: daysWithEvents.length === 0 ? (listNoEventsCustomComponentFn ? (jsx("div", { "data-ccid": listNoEventsCCID.current })) : (jsx("div", { className: "sx__list-no-events", children: $app.translate('No events') }))) : (daysWithEvents.map((day) => {
                ...
                return (jsxs("div", { className: "sx__list-day", "data-date": day.date, children: [listDayHeaderCustomComponentFn ? (jsx("div", { "data-ccid": dayHeaderCCID })) : (jsx("div", { className: "sx__list-day-header", children: jsx("div", { className: "sx__list-day-date", children: toJSDate(day.date).toLocaleDateString($app.config.locale.value, {
                                    weekday: 'long',
                                    year: 'numeric',
                                    month: 'long',
                                    day: 'numeric',
                                }) }) })), jsx("div", { className: "sx__list-day-events", children: day.events.map((event) => (jsx(ListEvent, { calendarEvent: event, dayDate: day.date, eventInstanceInfo: getEventInstanceInfo(event, day.date, $app.config.locale.value, $app.config.timezone.value), onEventClick: handleEventClick, onEventDoubleClick: handleEventDoubleClick, onEventKeyDown: handleEventKeyDown }, event.id))) }), jsx("div", { className: "sx__list-day-margin" })] }, day.date));
            })) }) }));
};
