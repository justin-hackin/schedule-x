...
const sxDateTimeStringRegex = /^(\d{4})-(\d{2})-(\d{2}) (0[0-9]|1[0-9]|2[0-3]):[0-5][0-9]$/;
...
class RRuleUpdater {
    ...
    updateByDay() {
        ...
        if (!this.rruleOptions.byday)
            return;
        const daysDifference = calculateDaysDifference(Temporal.PlainDate.from(this.dtstartOld), Temporal.PlainDate.from(this.dtstartNew));
    ...
    updateUntil() {
        ...
        this.rruleOptionsNew.until = isDateTime
            ? __deprecated__addMinutes(this.rruleOptionsNew.until, getDurationInMinutes(this.dtstartOld, this.dtstartNew)).toString()
            : addDays(Temporal.PlainDate.from(this.rruleOptionsNew.until), calculateDaysDifference(Temporal.PlainDate.from(this.dtstartOld), Temporal.PlainDate.from(this.dtstartNew))).toString();
    }
...
const dailyIterator = (dtstart, rruleOptions) => {
    ...
    return {
        next() {
            ...
            currentDate = __deprecated__addDaysToDateOrDateTime(currentDate, rruleOptions.interval);
            return { done: false, value: allDateTimes };
        },
    };
};
...
const formatDateTime = (year, month, day, hours, minutes) => {
    const monthStr = String(month + 1).padStart(DATE_PADDING_LENGTH, TIME_PADDING_CHAR);
    const dayStr = String(day).padStart(DATE_PADDING_LENGTH, TIME_PADDING_CHAR);
    if (hours !== undefined && minutes !== undefined) {
        const hoursStr = String(hours).padStart(DATE_PADDING_LENGTH, TIME_PADDING_CHAR);
        const minutesStr = String(minutes).padStart(DATE_PADDING_LENGTH, TIME_PADDING_CHAR);
...
const monthlyIteratorBymonthday = (dtstart, options) => {
    ...
    return {
        next() {
            ...
            const nextCurrentDateCandidate = __deprecated__addMonthsToDateOrDatetime(currentDate, options.interval);
            ...
            while (nextMonthDateCandidate !== options.bymonthday) {
                ...
                nextMonthDateCandidate = toIntegers(__deprecated__addMonthsToDateOrDatetime(currentDate, currentIntervalCandidate)).date;
            }
            currentDate = __deprecated__addMonthsToDateOrDatetime(currentDate, currentIntervalCandidate);
            return { done: false, value: allDateTimes };
        },
    };
};
const monthlyIteratorByday = (dtstart, options) => {
    ...
    return {
        next() {
            ...
            currentDate = __deprecated__addMonthsToDateOrDatetime(currentDate, options.interval);
            return { done: false, value: allDateTimes };
        },
    };
};
...
const calculateNextYearlyBydayOccurrence = (currentDate, rruleOptions) => {
    ...
    const targetDay = getNthWeekdayOfMonth(nextYear, targetMonth - 1, parsed.weekday, position);
    if (targetDay > 0) {
    ...
    return {
        next() {
            ...
            if (rruleOptions.byday && rruleOptions.byday.length > 0) {
                currentDate = calculateNextYearlyBydayOccurrence(currentDate, rruleOptions);
            }
            else {
...
class RecurrenceSet {
    ...
    getRecurrences() {
        const recurrences = new RRule(this.rrule, this.dtstart, this.dtend).getRecurrences();
    ...
    filterExdate(recurrences) {
        if (!this.exdate)
            return recurrences;
        return recurrences.filter((recurrence) => { var _a; return !((_a = this.exdate) === null || _a === void 0 ? void 0 : _a.has(recurrence.start)); });
    }
    getRrule() {
...
class DndUpdater {
    ...
    update(eventId, oldEventStart, newEventStart) {
        ...
        const newDtStart = oldEventStart instanceof Temporal.ZonedDateTime
            ? addMinutesToTemporal(eventToUpdate.start, getDurationInMinutesTemporal(oldEventStart, newEventStart))
            : addDays(eventToUpdate.start, calculateDaysDifference(Temporal.PlainDate.from(oldEventStart), Temporal.PlainDate.from(newEventStart)));
        const newDtEnd = oldEventStart instanceof Temporal.ZonedDateTime
            ? addMinutesToTemporal(eventToUpdate.end, getDurationInMinutesTemporal(oldEventStart, newEventStart))
            : addDays(eventToUpdate.end, calculateDaysDifference(Temporal.PlainDate.from(oldEventStart), Temporal.PlainDate.from(newEventStart)));
        // Update the original event
        recurrenceSet.updateDtstartAndDtend(parseTemporalToRFC5545(newDtStart), parseTemporalToRFC5545(newDtEnd));
        eventToUpdate.start = parseRFC5545ToTemporal(recurrenceSet.getDtstart(), this.$app.config.timezone.value);
        eventToUpdate.end = parseRFC5545ToTemporal(recurrenceSet.getDtend(), this.$app.config.timezone.value);
...
const createRecurrencesForEvent = ($app, calendarEvent, rrule, range, exdate) => {
    ...
    if (!recurrenceSet || recurrenceSet.length === 0)
        return [];
    if (recurrenceSet[0].start ===
...
const createRecurrencesForBackgroundEvent = ($app, backgroundEvent, rrule, range, exdate) => {
    // if there is no count or until in the rrule, set an until date to range.end but in rfc string format
    if (!rrule.includes('COUNT') && !rrule.includes('UNTIL')) {
        if (!rrule.endsWith(';'))
            rrule += ';';
    ...
    if (!recurrenceSet || recurrenceSet.length === 0)
        return [];
    if (parseSXToRFC5545(recurrenceSet[0].start) ===
...
class EventsFacadeImpl {
    ...
    set(events) {
        ...
        for (const event of events) {
            ...
            if (rrule &&
                ...
                newEventsList.push(...createRecurrencesForEvent(this.$app, newEvent, rrule, this.$app.calendarState.range.value, exdate));
            }
        }
    ...
    update(event) {
        ...
        const exdate = updatedEvent._getForeignProperties()
            .exdate;
        if (rrule &&
            typeof rrule === 'string' &&
            this.$app.calendarState.range.value) {
            copiedEvents.push(...createRecurrencesForEvent(this.$app, updatedEvent, rrule, this.$app.calendarState.range.value, exdate));
        }
...
class ResizeUpdater {
    ...
    getNewEventEnd(newEventEnd, eventToUpdate, oldEventEnd) {
        return newEventEnd instanceof Temporal.ZonedDateTime
            ? addMinutesToTemporal(eventToUpdate.end, getDurationInMinutesTemporal(oldEventEnd, newEventEnd))
...
class EventRecurrencePluginImpl {
    ...
    createRecurrencesForEvents() {
        ...
        $app.calendarEvents.list.value.forEach((event) => {
            ...
            if (rrule && this.validateRrule(event, rrule)) {
                recurrencesToCreate.push(...this.createRecurrencesForEvent(event, rrule, exdate));
            }
        });
        ...
        $app.calendarEvents.backgroundEvents.value.forEach((event) => {
            ...
            if (rrule && this.validateRrule(event, rrule)) {
                ...
                if (this.isInfiniteRecurringEvent(rrule)) {
                    rangeToUse = this.getExpandedRangeForInfiniteEvent(this.range, rrule, event.start, $app);
                }
                recurrencesToCreate.push(...createRecurrencesForBackgroundEvent($app, event, rrule, rangeToUse, exdate));
            }
        });
    ...
    validateRrule(event, rrule) {
        ...
        const logValidationFailure = (message) => {
            if ('id' in event) {
                console.warn(`[Schedule-X warning]: Recurrence set could not be created for event with id ${event.id}, because ${message}`);
            }
            else {
                console.warn(`[Schedule-X warning]: Recurrence set could not be created for background event with start ${event.start.toString()}, because ${message}`);
            }
        };
        if (rruleOptions.bymonthday) {
    ...
    getExpandedRangeForInfiniteEvent(range, rrule, eventStart, $app) {
        ...
        const eventStartZDT = eventStart instanceof Temporal.ZonedDateTime
            ? eventStart
        ...
        const endDate = minEndDate.epochNanoseconds > range.end.epochNanoseconds
            ? minEndDate
            : range.end;
        return {
            start: range.start,
    ...
    createRecurrencesForEvent(calendarEvent, rrule, exdate) {
        ...
        // For infinite recurring events, ensure minimum expansion
        if (this.isInfiniteRecurringEvent(rrule)) {
            rangeToUse = this.getExpandedRangeForInfiniteEvent(this.range, rrule, calendarEvent.start, $app);
        }
        return createRecurrencesForEvent($app, calendarEvent, rrule, rangeToUse, exdate);
    }
    ...
    setBackgroundEvents(backgroundEvents) {
        ...
        for (const event of backgroundEvents) {
            ...
            if (rrule && this.$app.calendarState.range.value) {
                newBackgroundEvents.push(...createRecurrencesForBackgroundEvent(this.$app, event, rrule, this.$app.calendarState.range.value, exdate));
            }
        }
...
export { createEventRecurrencePlugin, createEventsServicePlugin };
