  ...
  typeof exports === 'object' && typeof module !== 'undefined'
    ...
    : typeof define === 'function' && define.amd
      ...
      : ((global =
        ...
        factory((global.SXCalendarControls = {})))
  ...
  class CalendarControlsPluginImpl {
    constructor() {
      ...
      Object.defineProperty(this, 'getDayBoundaries', {
        ...
        writable: true,
        value: () => ({
          start: timeStringFromTimePoints(
            this.$app.config.dayBoundaries.value.start
          ),
          end: timeStringFromTimePoints(
            this.$app.config.dayBoundaries.value.end
          ),
        }),
      })
    ...
    setView(view) {
      const viewToSet = this.$app.config.views.value.find(
        (v) => v.name === view
      )
      if (!viewToSet)
        throw new Error(
    ...
    setViews(views) {
      ...
      if (!isCurrentViewInViews)
        throw new Error(
          `Currently active view is not in given views. Expected to find ${currentViewName} in ${views.map((view) => view.name).join(',')}`
        )
