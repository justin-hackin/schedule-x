...
import { useContext, useEffect, useRef, useMemo, createPortal } from 'preact/compat';
import { useState, useEffect as useEffect$1 } from 'preact/hooks';
...
function AppInput() {
    ...
    return (jsx(Fragment, { children: jsxs("div", { id: wrapperId, className: wrapperClasses.join(' '), children: [jsx("label", { htmlFor: inputId, className: "sx__time-input-label", children: (_a = $app.config.label.value) !== null && _a !== void 0 ? _a : $app.translate('Time') }), jsx("input", { value: $app.timePickerState.currentTimeDisplayedValue.value, readOnly: true, id: inputId, name: $app.config.name.value ? $app.config.name.value : 'time', className: "sx__time-picker-input", type: "text", onFocus: openPopup })] }) }));
}
...
function TimeInput({ initialValue, onChange, inputRef, nextTabIndexRef, validRange, }) {
    ...
    return (jsx("input", { ref: inputRef, maxLength: 2, className: "sx__time-input", type: "text", onKeyDown: incrementOrDecrementOnKeyDown, value: inputValue, onInput: handleInput, onBlur: handleOnBlur }));
}
...
function AppPopup() {
    ...
    const [hoursValue, setHoursValue] = useState($app.config.is12Hour.value
    ...
    return (jsxs("div", { className: classList.join(' '), style: $app.config.teleportTo.value ? fixedPositionStyle : undefined, children: [jsx("div", { className: "sx__time-picker-popup-label", children: $app.translate('Select time') }), jsxs("div", { className: "sx__time-picker-time-inputs", children: [jsx(TimeInput, { initialValue: hoursValue, onChange: (newHours) => setHoursValue(newHours), inputRef: hoursRef, nextTabIndexRef: minutesRef, validRange: $app.config.is12Hour.value ? [1, 12] : [0, 23] }), jsx("span", { className: "sx__time-picker-colon", children: ":" }), jsx(TimeInput, { initialValue: minutesValue, onChange: (newMinutes) => setMinutesValue(newMinutes), inputRef: minutesRef, validRange: [0, 59], nextTabIndexRef: OKButtonRef }), $app.config.is12Hour.value && (jsxs("div", { className: "sx__time-picker-12-hour-switches", children: [jsx("button", { type: "button", className: `sx__button sx__time-picker-12-hour-switch${$app.timePickerState.isAM.value ? ' is-selected' : ''}`, onClick: () => ($app.timePickerState.isAM.value = true), children: $app.translate('AM') }), jsx("button", { type: "button", className: `sx__button sx__time-picker-12-hour-switch${!$app.timePickerState.isAM.value ? ' is-selected' : ''}`, onClick: () => ($app.timePickerState.isAM.value = false), children: $app.translate('PM') })] }))] }), jsxs("div", { class: "sx__time-picker-actions", children: [jsx("button", { type: "button", class: "sx__button sx__time-picker-action sx__ripple sx__button-cancel", onClick: () => ($app.timePickerState.isOpen.value = false), children: $app.translate('Cancel') }), jsx("button", { ref: OKButtonRef, type: "button", class: "sx__button sx__time-picker-action sx__ripple sx__button-accept", onClick: handleAccept, children: $app.translate('OK') })] })] }));
}
...
function TimePickerWrapper({ $app }) {
    ...
    return (jsx(Fragment, { children: jsx("div", { className: classList.join(' '), children: jsxs(AppContext.Provider, { value: $app, children: [jsx(AppInput, {}), $app.timePickerState.isOpen.value && AppPopupJSX] }) }) }));
}
...
class TimePickerApp {
    ...
    render(el) {
        render(createElement(TimePickerWrapper, {
            $app: this.$app,
        }), el);
    }
    get value() {
...
const getTimePickerState = (config, is12HourClock) => {
    var _a;
    const currentTime = signal((_a = config.initialValue) !== null && _a !== void 0 ? _a : '00:00');
...
const createTimePickerAppContext = (config = {}, translateFn) => {
    ...
    return ({
        config: {
            onEscapeKeyDown: signal((_a = config.onEscapeKeyDown) !== null && _a !== void 0 ? _a : undefined),
            dark: signal((_b = config.dark) !== null && _b !== void 0 ? _b : false),
            placement: signal((_c = config.placement) !== null && _c !== void 0 ? _c : 'bottom-start'),
            teleportTo: signal((_d = config.teleportTo) !== null && _d !== void 0 ? _d : null),
            label: signal((_e = config.label) !== null && _e !== void 0 ? _e : null),
            is12Hour: signal((_f = config.is12Hour) !== null && _f !== void 0 ? _f : false),
            name: signal((_g = config.name) !== null && _g !== void 0 ? _g : ''),
        },
